...
/** V2 底部导航的两页：今天 / 痕迹（「开始」「我的」是动作槽，不是分页） */
export type Tab = 'today' | 'traces';
...
export interface UIState {
  ...
  historyOpen: boolean;
  /** 正在编辑的记录(id);从痕迹页进入编辑态时指向该记录,新建为 null */
...
export type AppAction =
  ...
  | { type: 'addRecord'; text: string; images?: RecordImage[]; refined?: string; outcome?: MemoryRecord['outcome']; createdAt?: number; linkedDOId?: string }
  ...
  /** 合并更新设置(theme/background/ai/onboarded) */
